...
TEST_CASE("Brent with y = x*x -3x -6")
{
    ...
    SECTION("left root") { CHECK(Brent::findRoot(lin, Range<double>(-2., 3.)) == Catch::Approx(-1.3722813204)); }

    SECTION("right root") { CHECK(Brent::findRoot(lin, Range<double>(0., 5.)) == Catch::Approx(4.3722813204)); }
}
